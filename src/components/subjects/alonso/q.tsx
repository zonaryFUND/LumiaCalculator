...
const q: React.FC<SubjectSkillProps> = props => (
    <>
        前方に磁力の手を突き出して<Damage skill="Q" constants={Constants.Q.damage} {...props} />の
        スキルダメージを与え、的中した敵と<span className={style.emphasis}>磁力線</span>でつなげます。<br />
        <br />
        アロンソが<span className={style.emphasis}>磁力線</span>でつながった対象に
        向かって移動する場合、移動速度が{Constants.Q.movement_speed}％増加し、
        距離が近いほど最大{Constants.Q.near_movement_speed[props.config.skillLevels.Q]}％まで増加します。<br />
        <br />
        <span className={style.emphasis}>磁力線</span>でつながった敵を
        基本攻撃で攻撃すると磁力線が解除され、<BasicAttackDamage {...props} />の
        スキルダメージを与えて{Constants.Q.stun}秒間気絶させます。
    </>
);
