...
const r: React.FC<SubjectSkillProps> = props => (
    <>
        ...
        <br />
        <span className={style.emphasis}>シャマシュの法典</span>：指定した方向にシャマシュの法典を開きます。
        シャマシュの法典は衝突した敵に<Damage skill="Q" constants={Constants.Q.damage} {...props} />
        ...
        <span className={style.emphasis}>秘められた力</span>のクールダウンの{Constants.R.Q.cooldown_reduction}％が
        返されます。<br />
        <br />
        <span className={style.emphasis}>バビロンのさいころ</span>：指定した地点にバビロンのさいころを召喚して
        境界線に障壁を作り、障壁に触れた敵を{Constants.R.W.bind[props.config.skillLevels.R]}秒間束縛させます。<br />
        障壁は{Constants.R.W.duration}秒間時計回りの順番で破壊されながら敵に
        <Damage skill="R" constants={Constants.R.W.damage} {...props} />のスキルダメージを与え、
        さいころの領域内の敵には<Damage skill="W" constants={Constants.W.damage} {...props} />のスキルダメージを与えて
        ...
        <br />
        <span className={style.emphasis}>ニムルドの門</span>：使用距離が{Constants.R.E.range[props.config.skillLevels.R]}m増加し、
        指定した位置と自分の位置にニムルドの石碑を落として敵に<Damage skill="E" constants={Constants.E.damage} {...props} />
