import Constants from "./constants.json";
import { TooltipInfo } from "../dictionary";

export const code = 1066300;

export const info: TooltipInfo = {
    skill: "W",
    consumption: {
        type: "sp",
        value: Constants.W.sp_cost
    },
    cooldown: Constants.W.cooldown,
    values: ({ skillLevel, showEquation }) => ({
        0: showEquation ? Constants.W.damage.base[skillLevel] : Constants.W.damage,
        1: Constants.W.slow.duration,
        2: `${Constants.W.slow.effect}%`,
        3: Constants.W.duration,
        4: showEquation ? Constants.W.vanish_damage.base[skillLevel] : Constants.W.vanish_damage,
        5: showEquation ? `${Constants.W.damage.amp}%` : Constants.W.stun,
        6: `${Constants.W.vanish_damage.amp}%`,
        7: Constants.W.stun
    }),
    expansion: () => ({
        enumeratedValues: [
            {labelIntlID: "ToolTipType/Damage", values: Constants.W.damage.base},
            {labelIntlID: "ToolTipType/AdditionalDamage", values: Constants.W.vanish_damage.base},
            {labelIntlID: "ToolTipType/Cost", values: Constants.W.sp_cost},
            {labelIntlID: "ToolTipType/CoolTime", values: Constants.W.cooldown},
        ]  
    })
}
